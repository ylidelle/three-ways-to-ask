...
internal activations, a self-report survey of seven constructs each asked in two
independently cloned paraphrase forms, and the model's behaviour on a
neutral probe. Across 20 matched triplets of 50-turn conversations on
`gemma-3-12b-it`, **the three agree with each other at close to chance** (mean
Cohen's κ = +0.059), behaving as near-independent instruments rather than
redundant views of one signal. Where all three concur, accuracy is 0.737 against
0.667 for the best single method, but that gap does not survive an exact test over
all 2²⁰ label assignments (p = 0.053); we report it as a null result, together
with our first implementation of that test, which did not refit and called it
significant.

We also report what the comparison cannot support. Because our conditions differ
by prompt, a classifier reading only the input text separates them perfectly
(1.000), so arm-decoding accuracy is a manipulation check for **every** method
here, self-report included. And because an interpretability null is
uninterpretable without a sensitivity floor, we calibrated the same instrument on
content the model was told to withhold: the concealed concept stayed detectable on
4 of 8 targets, faintly, with a no-concept control silent on 8 of 8. Code,
instrument and all run artefacts are released.
...
Two auxiliary tools check this manuscript against the artefacts: one re-reads
every inferential number from the file that produced it and requires it at the
table cell that claims it, the other requires each quoted source sentence to
appear as a complete quoted span. **Both are designed to catch our own drift:
a stale number, or a comma edited out of a citation during a style pass. Both
have caught exactly that during this work.** Neither should be read as proof
against a document edited to deceive them: our external reviewer defeated three
successive versions of each, and every attack he found is now a regression test
in the repository. **We cite them as evidence of care, not as clearance.**
