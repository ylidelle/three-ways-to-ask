...
including ordinary copy-edit errors rather than only adversarial ones. Each
attack is committed as a case in the corresponding `--selftest` (14 for the
number checker, 11 for the quote guard), so a later edit that removes a fix
fails the suite rather than passing quietly. **We cite them as evidence of care,
not as clearance.**
