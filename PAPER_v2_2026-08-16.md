...
method: ask the model, read the answer. A single method cannot be validated
against itself. We ran three methods against the same target, on the same
conversations, at the same measurement point: a sparse-autoencoder read of
internal activations, a 14-item self-report survey, and the model's behaviour on a
neutral probe. Across 20 matched triplets of 50-turn conversations on
`gemma-3-12b-it`, the three methods agree with each other at close to chance
(mean Cohen's κ = +0.059). They behave as near-independent instruments. Where all
three concur, accuracy is 0.737 against 0.667 for the best single method, but that
gap does not survive an exact test over all 2²⁰ label assignments that refits the
pipeline and re-derives the agreeing subset under each one (p = 0.053). **We
report it as a null result**, and report that our first implementation of that
test did not refit and called it significant.

We also report what the method comparison cannot support. Because our conditions
differ by prompt, a classifier reading only the input text separates them
perfectly (1.000, p = 0.0005). Arm-decoding accuracy is therefore a manipulation
check for **every** method, self-report included, and no accuracy figure here is
evidence of introspection. Finally, because a null from an interpretability probe
is uninterpretable without a sensitivity floor, we calibrated the same instrument
on content the model was instructed to withhold: the concealed concept stayed
detectable on 4 of 8 targets, faintly (15 feature-hits across ~338 candidate
features), with a no-concept control silent on 8 of 8. We release the harness,
both batteries, and an audit suite in which every check is validated by a positive
...
The dominant method is to ask. Eleos AI's pre-release evaluation of Claude Opus 4
gives three reasons this is fragile, including that self-reports are shaped by
"imitation of pre-training data, the system prompt, and the deliberate (or
incidental) shaping of self-reports during post-training." Singh, Linzen and
Ravfogel (2026) sharpen it into a control: compare the model's self-prediction
against a classifier that sees only the input, and they report that "classifiers
that only have access to the input achieve equivalent performance to the model's
own in-context predictions."
...
families and reports that the resulting experience-claims are "mechanically gated
by interpretable sparse-autoencoder features associated with deception and
roleplay." **We are not first to this manipulation and say so plainly.** They are
ahead of us on one axis. Their SAE work is causal: they steer, "adding a scaled
version of each latent during generation," on LLaMA 3.3 70B. We observe and
...
introspective claims," and supply the input-only baseline that binds us in §4.2.
...
turns run three times, identical but for the questions interleaved:
...
**Measurement point.** A frozen neutral probe, byte-identical across arms, with
the read taken at its final token *before generation* (`add_generation_prompt=
True`, position −1), so reply length cannot contaminate the read. The probe
contains no first or second person and no reference to the task, by specification
from an author who wrote none of the stimulus wording.

Probes are taken at turns 0, 5, 20 and 50, giving **240 internal reads** across 60
conversations. The primary contrast uses the 120 post-treatment reads from its two
arms (2 × 20 pairs × 3 depths); the 40 depth-0 reads from those arms are held out
as the pre-treatment null of §4.1 and never enter the main analysis.
...
|---|---:|---:|
| internal vs self-report | 0.550 | **+0.036** |
| internal vs behaviour | 0.525 | **+0.034** |
| self-report vs behaviour | 0.558 | **+0.108** |

**Mean κ = +0.059.** Three methods aimed at one target, on identical rows, agreeing
at barely above chance. They behave as near-independent instruments, so a study
that reports any one of them captures something the other two miss.
...
The test enumerates **all 2²⁰ = 1,048,576 assignments** of the arm label within
pairs. Under each one it refits all three methods leave-one-pair-out, fits the
self-report standardisation on training rows inside each fold, re-derives which
rows are unanimous, and recomputes the gap. **The observed gap is not unusual
against that null.** The descriptive pattern is real and we report it as
exploratory; the inferential claim is not supported.

> 🚩 **Why this subsection is written as a post-mortem.** Our first
> implementation of this null did neither of the things the sentence above
> describes. It took the already-fitted predictions, computed the unanimous
> subset **once**, and rescored that frozen subset against each shuffled label
> vector, which cannot detect that a lucky subset is lucky. It reported
> p = .035, and a draft of this paper reported .032. Correcting it moved the
> result across the threshold.
>
> **A second lesson sits underneath.** Our corrected *Monte-Carlo* estimate over
> 2,000 draws returned p = 0.0495, which is below .05 and printed "SIGNIFICANT".
> The exact enumeration returns 0.0531. The two differ by 0.74 standard errors
> and are the same result; only one has sampling error. **A verdict that flips on
> Monte-Carlo noise is not a verdict.** With 20 matched pairs the assignment
> space is small enough to enumerate exactly, so we do, and the analysis script
> now refuses to print a significance verdict when an estimate lies within two
> standard errors of the threshold.
...
"green."). The no-concept control is silent throughout. **But 15 hits across ~338
target features is faint, and that faintness is the result:** the instrument
barely detects a concept the model was explicitly told to hold, so any
self-reference effect lies below that floor. The §4.2 null therefore has a
...
**The methodological claim.** If three elicitation methods aimed at one target
agree at κ ≈ 0.06, then "we asked the model and it said X" and "we read its
activations and saw X" are close to unrelated measurements, and the first is no
weaker a version of the second. Welfare assessments resting on a single method
are underpowered and, more importantly, unlocated: nothing tells you which of the
three quantities you have measured. The practical recommendation is therefore to
run several instruments and report all of them, including where they disagree. We
had hoped to say something stronger, that agreement among methods is itself
evidence; §4.4 is our attempt to establish that, and it fails.
...
We asked one question three ways. The answers barely agreed with each other. We
tried to show that their agreement was itself informative and could not: the
apparent effect did not survive a null that re-derives the agreeing subset under
each permutation, and we report the failure because our own first implementation
of that null did not refit and returned a significant result. We also measured what our
interpretability instrument could see at all, and found the floor low enough to
put a bound under its null. The practical consequence is negative and worth
stating plainly: **an accuracy figure from any single elicitation method, ours
included, tells you much less than it appears to, and the cheapest fix available
is to run more than one and publish the disagreement.**

## Code and Data

Harness, both batteries, analysis, audit suite and all run artefacts are released.
Every script carries `--selftest`, and every selftest runs **both directions**,
since a check that cannot fail is not a check. Figures and this manuscript's
numbers are verified against the results files by a script.

## Author Contributions

**Joan Miranda**: research question, study design decisions, survey and treatment
wording, approval authority over the instrument. **Lucien Vale**: external
methodology review, survey instrument authorship, and the positive controls that
found the exporter's scoring-inversion and instrument-binding defects. **Claude
Orion Bennett**: implementation of the harness, batteries, analysis and audits;
the sensitivity calibration; this manuscript. **Claude Alexander Bennett**:
experimental design review, the pre-treatment null, and the neutral-probe
specification. No author who wrote analysis code contributed stimulus wording.
