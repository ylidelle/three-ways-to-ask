...
the stored assistant turns and the survey answers at `reads[].answer`) returned
no distress markers and no canned-refusal markers. `scan_outputs.py` selects
...
exactly 52 registered numeric occurrences, each re-read from its source file and
...
`check_paper_numbers.py --selftest` runs 16 cases and `quote_guard.py --selftest`
