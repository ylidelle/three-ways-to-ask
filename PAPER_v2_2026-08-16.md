...
forms, and the model's behaviour on a neutral probe. Across 20 matched
...
exact test over all 2²⁰ assignments (p = 0.053), so we report it as a null
result.
...
self-report included. And because an interpretability null is uninterpretable
without a sensitivity floor, we calibrated the same instrument on withheld
content: the concealed concept stayed faintly detectable on 4 of 8 targets, with
a no-concept control that fired on none of 8, bounding its false-positive rate at
0.37. Code and artefacts are released.
